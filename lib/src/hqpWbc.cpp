/**
 * @file HqpWbc.cpp
 * @brief Function implementation part of class HqpWbc
 * @author Yan Xie
 * @date 2020-09-17
 * @version alpha
...
HqpWbc::HqpWbc(const Wbc & wbc_foo):Wbc(wbc_foo){
    createNewQP();
    resizeQPMatrixVector();
}
...
bool HqpWbc::getNwsr(std::vector<int> & nWSR_res){
    for (int i = 0; i != nLevel; i++){
...
bool HqpWbc::getOptCost(std::vector<double> & cost_opt){
    for (int i = 0; i != nLevel; i++){
